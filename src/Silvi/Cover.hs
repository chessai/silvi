...
-- list of fake value generators.  It's @[Gen a]@ instead of @Gen [a]@
-- because we don't want to have to evaluate the 'Gen' monad to work with
-- coverage lists.
newtype Coverage a = Coverage { unCoverage :: [Gen a] }
  deriving (Functor)
...
instance (GCover a, GCover b) => GCover (a G.:*: b) where
    genericCover = zipWith (liftM2 (G.:*:))
      ...
      where
        acover = genericCover :: [Gen (a x)]
        alen = length acover
        bcover = genericCover :: [Gen (b x)]
        blen = length bcover
        newlen = max alen blen
